...
namespace chips {
    ...
    private:
        ...
        std::unique_ptr<rvalue<dft,expression_env::PRIMITIVE>> m_default_value;
        std::unique_ptr<dataflow_declaration<dft,statement_env::DEFINITION>> m_declaration;
        std::string m_name;

        public:
            function_parameter(std::unique_ptr<dataflow_declaration<dft,statement_env::DEFINITION>> declaration,
                               std::string identifier, std::unique_ptr<rvalue<dft,expression_env::PRIMITIVE>> default_value)
                               : m_declaration(std::move(declaration)), m_name(identifier), m_default_value(std::move(default_value)){}

            
            function_parameter(std::unique_ptr<dataflow_declaration<dft,statement_env::DEFINITION>> declaration,
                               std::string identifier)
                               : m_declaration(std::move(declaration)), m_name(identifier){}

            function_parameter(){
                std::cout << "FUNCTION_PARAMTER EMPTY" << std::endl;
            };

            dataflow_declaration<dft,statement_env::DEFINITION>* get_dataflow_declaration() { return m_declaration.get(); }
            std::string get_identifier() { return m_name; }
            std::unique_ptr<rvalue<dft,expression_env::PRIMITIVE>>* get_default_value() { return m_default_value.get(); }

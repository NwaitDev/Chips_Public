...
public:
    virtual ~s_statement_node() = default;

    // Primary statement discriminator (non-configuration statements reuse STATEMENT_TYPE)
    virtual STATEMENT_TYPE get_type() = 0;

    // Convenience wrapper used by writer/visitors on const pointers
    virtual STATEMENT_TYPE get_statement_type() const {
        return const_cast<s_statement_node*>(this)->get_type();
    }
};
...
public:
    ...
    const std::vector<std::unique_ptr<s_statement_node>>& get_statements() const { return statements; }

    STATEMENT_TYPE get_statement_type() const { return INST_ST; }
...
public:
    s_loop_node(std::string ident1, std::unique_ptr<expression_node> loop_expr, std::unique_ptr<s_statements_node> stts)
        : ident1(std::move(ident1)), loop_expr(std::move(loop_expr)), stts(std::move(stts)) {}
...
public:
    functionnal_block_instanciation_node(std::string ident1, std::unique_ptr<suffixes_node> suff, std::string ident2)
        : ident1(std::move(ident1)), suff(std::move(suff)), ident2(std::move(ident2)) {}
...
public:
    plugging_node(std::unique_ptr<block_node> block, std::string identifier, std::unique_ptr<suffixes_node> suff, std::unique_ptr<expression_node> expr)
        : block(std::move(block)), identifier(std::move(identifier)), suff(std::move(suff)), expr(std::move(expr)) {}
...
public:
    implements_node(std::string ident1, std::unique_ptr<suffixes_node> suffixes1, std::string ident2, std::unique_ptr<suffixes_node> suffixes2, std::string ident3)
        : ident1(std::move(ident1)), suffixes1(std::move(suffixes1)), ident2(std::move(ident2)), suffixes2(std::move(suffixes2)), ident3(std::move(ident3)) {}

...
tests_exernal() {
  ...
  $OUTPUT_DIR/build/system --silent > $OUTPUT_DIR/trace-silent || fail "Executable returned an error"
  ...
  let "success+=$(grep "choose.*A.p1" -c $OUTPUT_DIR/trace)"
  let "success+=$(grep "choose.*B.p1" -c $OUTPUT_DIR/trace)"
  let "success+=$(grep "choose.*A.p2" -c $OUTPUT_DIR/trace)"
  let "success+=$(grep "choose.*B.p2" -c $OUTPUT_DIR/trace)"

  let "success+=$(grep "choose.*A.p3" -c $OUTPUT_DIR/trace)"
  let "success+=$(grep "choose.*B.p3" -c $OUTPUT_DIR/trace)"
  let "success+=$(grep "choose.*A.p4" -c $OUTPUT_DIR/trace)"
  let "success+=$(grep "choose.*A.p4" -c $OUTPUT_DIR/trace)"

  let "success+=$(grep "choose.*conn.*A.p.*B.p" -c $OUTPUT_DIR/trace)"
  
  let "success+=$(grep "state #405:.*deadlock" -c $OUTPUT_DIR/trace)"

  let "success+=$(grep "sync 1 and 2" -c $OUTPUT_DIR/trace-silent)"
...
tests_exernal_error() {
  ...
  $OUTPUT_DIR/build/system --silent > $OUTPUT_DIR/trace-silent
  assertEquals 6 $?
...
test_petri_net() {
  ...
  grep -q 'sync:.*petriNet.sync' $OUTPUT_DIR/trace && \
...
test_simple10counter() {
  ...
  LAST_EXEC_LINE=$($OUTPUT_DIR/build/system |tail -n 1)

  [ ${PIPESTATUS[0]} -eq "0" ] || fail "Executable returned an error"

  assertEquals "[BIP ENGINE]: state #0: deadlock!" "$LAST_EXEC_LINE"
}

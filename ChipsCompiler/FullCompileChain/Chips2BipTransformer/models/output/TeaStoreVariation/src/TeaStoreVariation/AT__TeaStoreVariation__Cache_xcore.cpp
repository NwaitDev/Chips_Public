...
AT__TeaStoreVariation__Cache_xcore::AT__TeaStoreVariation__Cache_xcore(const string &name , AtomIPort__TeaStoreVariation__port_xt &_iport_decl__start_aipd, AtomIPort__TeaStoreVariation__port_xt &_iport_decl__then_aipd, AtomIPort__TeaStoreVariation__bool_xport_xt &_iport_decl__send_aipd, AtomIPort__TeaStoreVariation__intint_xport_xt &_iport_decl__recv_aipd
                         ...
                         , bool _comp_param__starts) :  ComponentItf(name, ATOM), Atom(name) , _iport_decl__start_aipd(_iport_decl__start_aipd), _iport_decl_pv__start_aipd(), _iport_decl__then_aipd(_iport_decl__then_aipd), _iport_decl_pv__then_aipd(), _iport_decl__send_aipd(_iport_decl__send_aipd), _iport_decl_pv__send_aipd(_id__foundInCache_aidd), _iport_decl__recv_aipd(_iport_decl__recv_aipd), _iport_decl_pv__recv_aipd(_id__requestedData_aidd, _id__cacheSize_aidd), _eport_decl__send_aepd(_eport_decl__send_aepd), _eport_decl__recv_aepd(_eport_decl__recv_aepd), _comp_param__starts(_comp_param__starts), _transguard__1(false), _transguard__2(false), _transguard__3(false), _transguard__4(false), _transguard__5(false), __previous(-1) {
...
BipError& AT__TeaStoreVariation__Cache_xcore::execute(PortValue &portValue) {
...
BipError& AT__TeaStoreVariation__Cache_xcore::execute(AtomExternalPort &port) {
   BipError *ret = &BipError::NoError;
...
BipError& AT__TeaStoreVariation__Cache_xcore::initialize() {
    BipError *ret = &BipError::NoError;
...
string AT__TeaStoreVariation__Cache_xcore::toString() const {
    ostringstream oss;
    bool first=true;
    if (atINIT()) {
      if (!first) oss << ", ";
    ...
    oss << "requestedData_aidd=" << _id__requestedData_aidd;
    if (first) first = false;
    else oss << std::endl;
    oss << "cacheSize_aidd=" << _id__cacheSize_aidd;
    if (first) first = false;
    else oss << std::endl;
    oss << "maxCacheSize_aidd=" << _id__maxCacheSize_aidd;
    if (first) first = false;
    else oss << std::endl;
    oss << "minCacheSize_aidd=" << _id__minCacheSize_aidd;
    if (first) first = false;
    else oss << std::endl;
    oss << "cache_aidd=" << _id__cache_aidd;
    if (first) first = false;
    else oss << std::endl;
    oss << "newestDataIndex_aidd=" << _id__newestDataIndex_aidd;
    if (first) first = false;
    else oss << std::endl;
    oss << "found_aidd=" << _id__found_aidd;
    if (first) first = false;
    else oss << std::endl;
    oss << "foundIndex_aidd=" << _id__foundIndex_aidd;
    if (first) first = false;
    else oss << std::endl;
    oss << "foundInCache_aidd=" << _id__foundInCache_aidd;

    return oss.str(); 
}

BipError& AT__TeaStoreVariation__Cache_xcore::update() {
    BipError *ret = &BipError::NoError;
...
BipError &AT__TeaStoreVariation__Cache_xcore::executeInternalTransitions() {
    BipError &ret = BipError::NoError;
...
AT__TeaStoreVariation__Cache_xcore::~AT__TeaStoreVariation__Cache_xcore() {
}

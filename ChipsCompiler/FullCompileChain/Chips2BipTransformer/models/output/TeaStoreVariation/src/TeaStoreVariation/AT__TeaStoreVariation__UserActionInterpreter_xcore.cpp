...
#include <TeaStoreVariation/AT__TeaStoreVariation__UserActionInterpreter_xcore.hpp>
...
bool AT__TeaStoreVariation__UserActionInterpreter_xcore::atINIT () const {
    return __statesbv[ 0/(8*sizeof(int))] & 1<< (0%(8*sizeof(int)));
}

bool AT__TeaStoreVariation__UserActionInterpreter_xcore::toINIT () {
    if (atINIT()) return false;
    __statesbv[ 0/(8*sizeof(int))] |= 1<< (0%(8*sizeof(int)));
    return true;
}

bool AT__TeaStoreVariation__UserActionInterpreter_xcore::fromINIT () {
    if (!atINIT()) return false;
    __statesbv[ 0/(8*sizeof(int))] &= ~(1<< (0%(8*sizeof(int))));
    return true;
}

bool AT__TeaStoreVariation__UserActionInterpreter_xcore::atRECEIVE () const {
    return __statesbv[ 1/(8*sizeof(int))] & 1<< (1%(8*sizeof(int)));
}

bool AT__TeaStoreVariation__UserActionInterpreter_xcore::toRECEIVE () {
    if (atRECEIVE()) return false;
    __statesbv[ 1/(8*sizeof(int))] |= 1<< (1%(8*sizeof(int)));
    return true;
}

bool AT__TeaStoreVariation__UserActionInterpreter_xcore::fromRECEIVE () {
    if (!atRECEIVE()) return false;
    __statesbv[ 1/(8*sizeof(int))] &= ~(1<< (1%(8*sizeof(int))));
    return true;
}

bool AT__TeaStoreVariation__UserActionInterpreter_xcore::atTHEN () const {
    return __statesbv[ 2/(8*sizeof(int))] & 1<< (2%(8*sizeof(int)));
}

bool AT__TeaStoreVariation__UserActionInterpreter_xcore::toTHEN () {
    if (atTHEN()) return false;
    __statesbv[ 2/(8*sizeof(int))] |= 1<< (2%(8*sizeof(int)));
    return true;
}

bool AT__TeaStoreVariation__UserActionInterpreter_xcore::fromTHEN () {
    if (!atTHEN()) return false;
    __statesbv[ 2/(8*sizeof(int))] &= ~(1<< (2%(8*sizeof(int))));
    return true;
}

bool AT__TeaStoreVariation__UserActionInterpreter_xcore::atSEND () const {
    return __statesbv[ 3/(8*sizeof(int))] & 1<< (3%(8*sizeof(int)));
}

bool AT__TeaStoreVariation__UserActionInterpreter_xcore::toSEND () {
    if (atSEND()) return false;
    __statesbv[ 3/(8*sizeof(int))] |= 1<< (3%(8*sizeof(int)));
    return true;
}

bool AT__TeaStoreVariation__UserActionInterpreter_xcore::fromSEND () {
    if (!atSEND()) return false;
    __statesbv[ 3/(8*sizeof(int))] &= ~(1<< (3%(8*sizeof(int))));
    return true;
}

AT__TeaStoreVariation__UserActionInterpreter_xcore::AT__TeaStoreVariation__UserActionInterpreter_xcore(const string &name , AtomIPort__TeaStoreVariation__port_xt &_iport_decl__start_aipd, AtomIPort__TeaStoreVariation__port_xt &_iport_decl__then_aipd, AtomIPort__TeaStoreVariation__boolintint_xport_xt &_iport_decl__send_aipd, AtomIPort__TeaStoreVariation__boolintint_xport_xt &_iport_decl__recv_aipd
                         ...
                         , bool _comp_param__starts) :  ComponentItf(name, ATOM), Atom(name) , _iport_decl__start_aipd(_iport_decl__start_aipd), _iport_decl_pv__start_aipd(), _iport_decl__then_aipd(_iport_decl__then_aipd), _iport_decl_pv__then_aipd(), _iport_decl__send_aipd(_iport_decl__send_aipd), _iport_decl_pv__send_aipd(_id__authProvided_aidd, _id__nbData_aidd, _id__reqID_aidd), _iport_decl__recv_aipd(_iport_decl__recv_aipd), _iport_decl_pv__recv_aipd(_id__authDataRcvd_aidd, _id__nbDataRcvd_aidd, _id__userIDRcvd_aidd), _eport_decl__send_aepd(_eport_decl__send_aepd), _eport_decl__recv_aepd(_eport_decl__recv_aepd), _comp_param__starts(_comp_param__starts), _transguard__1(false), _transguard__2(false), _transguard__3(false), _transguard__4(false), _transguard__5(false), __previous(-1) {
...
BipError& AT__TeaStoreVariation__UserActionInterpreter_xcore::execute(PortValue &portValue) {
...
BipError& AT__TeaStoreVariation__UserActionInterpreter_xcore::execute(AtomExternalPort &port) {
   BipError *ret = &BipError::NoError;
...
BipError& AT__TeaStoreVariation__UserActionInterpreter_xcore::initialize() {
    BipError *ret = &BipError::NoError;
...
string AT__TeaStoreVariation__UserActionInterpreter_xcore::toString() const {
    ostringstream oss;
    bool first=true;
    if (atINIT()) {
      if (!first) oss << ", ";
    ...
    oss << "authDataRcvd_aidd=" << _id__authDataRcvd_aidd;
    if (first) first = false;
    else oss << std::endl;
    oss << "nbDataRcvd_aidd=" << _id__nbDataRcvd_aidd;
    if (first) first = false;
    else oss << std::endl;
    oss << "userIDRcvd_aidd=" << _id__userIDRcvd_aidd;
    if (first) first = false;
    else oss << std::endl;
    oss << "authProvided_aidd=" << _id__authProvided_aidd;
    if (first) first = false;
    else oss << std::endl;
    oss << "nbData_aidd=" << _id__nbData_aidd;
    if (first) first = false;
    else oss << std::endl;
    oss << "reqID_aidd=" << _id__reqID_aidd;

    return oss.str(); 
}

BipError& AT__TeaStoreVariation__UserActionInterpreter_xcore::update() {
    BipError *ret = &BipError::NoError;
...
BipError &AT__TeaStoreVariation__UserActionInterpreter_xcore::executeInternalTransitions() {
    BipError &ret = BipError::NoError;

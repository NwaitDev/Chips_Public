...
#include <TeaStoreVariation/AT__TeaStoreVariation__bool_xconstant_xt.hpp>
...
bool AT__TeaStoreVariation__bool_xconstant_xt::atDATA_SENDING () const {
    return __statesbv[ 0/(8*sizeof(int))] & 1<< (0%(8*sizeof(int)));
}

bool AT__TeaStoreVariation__bool_xconstant_xt::toDATA_SENDING () {
    if (atDATA_SENDING()) return false;
    __statesbv[ 0/(8*sizeof(int))] |= 1<< (0%(8*sizeof(int)));
    return true;
}

bool AT__TeaStoreVariation__bool_xconstant_xt::fromDATA_SENDING () {
    if (!atDATA_SENDING()) return false;
    __statesbv[ 0/(8*sizeof(int))] &= ~(1<< (0%(8*sizeof(int))));
    return true;
}

AT__TeaStoreVariation__bool_xconstant_xt::AT__TeaStoreVariation__bool_xconstant_xt(const string &name , AtomIPort__TeaStoreVariation__bool_xport_xt &_iport_decl__output_aipd
                         
                         , AtomEPort__TeaStoreVariation__bool_xport_xt &_eport_decl__output_aepd
                         
                         ) :  ComponentItf(name, ATOM), Atom(name) , _iport_decl__output_aipd(_iport_decl__output_aipd), _iport_decl_pv__output_aipd(_id__bool_value_aidd), _eport_decl__output_aepd(_eport_decl__output_aepd), _transguard__1(false), __previous(-1) {
    this->addInternalPort(_iport_decl__output_aipd);
    // export port
    this->addPort(_eport_decl__output_aepd);
}

BipError& AT__TeaStoreVariation__bool_xconstant_xt::execute(PortValue &portValue) {
...
BipError& AT__TeaStoreVariation__bool_xconstant_xt::execute(AtomExternalPort &port) {
   BipError *ret = &BipError::NoError;
...
BipError& AT__TeaStoreVariation__bool_xconstant_xt::initialize() {
    BipError *ret = &BipError::NoError;
...
string AT__TeaStoreVariation__bool_xconstant_xt::toString() const {
    ostringstream oss;
    bool first=true;
    if (atDATA_SENDING()) {
      if (!first) oss << ", ";
    ...
    oss << "bool_value_aidd=" << _id__bool_value_aidd;

    return oss.str(); 
}

BipError& AT__TeaStoreVariation__bool_xconstant_xt::update() {
    BipError *ret = &BipError::NoError;
...
BipError &AT__TeaStoreVariation__bool_xconstant_xt::executeInternalTransitions() {
    BipError &ret = BipError::NoError;
...
AT__TeaStoreVariation__bool_xconstant_xt::~AT__TeaStoreVariation__bool_xconstant_xt() {
}

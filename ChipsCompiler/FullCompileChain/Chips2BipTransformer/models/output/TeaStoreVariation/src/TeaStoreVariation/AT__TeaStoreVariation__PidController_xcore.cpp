...
bool AT__TeaStoreVariation__PidController_xcore::atINIT () const {
    return __statesbv[ 0/(8*sizeof(int))] & 1<< (0%(8*sizeof(int)));
}

bool AT__TeaStoreVariation__PidController_xcore::toINIT () {
    if (atINIT()) return false;
    __statesbv[ 0/(8*sizeof(int))] |= 1<< (0%(8*sizeof(int)));
    return true;
}

bool AT__TeaStoreVariation__PidController_xcore::fromINIT () {
    if (!atINIT()) return false;
    __statesbv[ 0/(8*sizeof(int))] &= ~(1<< (0%(8*sizeof(int))));
    return true;
}

bool AT__TeaStoreVariation__PidController_xcore::atRECEIVE () const {
    return __statesbv[ 1/(8*sizeof(int))] & 1<< (1%(8*sizeof(int)));
}

bool AT__TeaStoreVariation__PidController_xcore::toRECEIVE () {
    if (atRECEIVE()) return false;
    __statesbv[ 1/(8*sizeof(int))] |= 1<< (1%(8*sizeof(int)));
    return true;
}

bool AT__TeaStoreVariation__PidController_xcore::fromRECEIVE () {
    if (!atRECEIVE()) return false;
    __statesbv[ 1/(8*sizeof(int))] &= ~(1<< (1%(8*sizeof(int))));
    return true;
}

bool AT__TeaStoreVariation__PidController_xcore::atTHEN () const {
    return __statesbv[ 2/(8*sizeof(int))] & 1<< (2%(8*sizeof(int)));
}

bool AT__TeaStoreVariation__PidController_xcore::toTHEN () {
    if (atTHEN()) return false;
    __statesbv[ 2/(8*sizeof(int))] |= 1<< (2%(8*sizeof(int)));
    return true;
}

bool AT__TeaStoreVariation__PidController_xcore::fromTHEN () {
    if (!atTHEN()) return false;
    __statesbv[ 2/(8*sizeof(int))] &= ~(1<< (2%(8*sizeof(int))));
    return true;
}

bool AT__TeaStoreVariation__PidController_xcore::atSEND () const {
    return __statesbv[ 3/(8*sizeof(int))] & 1<< (3%(8*sizeof(int)));
}

bool AT__TeaStoreVariation__PidController_xcore::toSEND () {
    if (atSEND()) return false;
    __statesbv[ 3/(8*sizeof(int))] |= 1<< (3%(8*sizeof(int)));
    return true;
}

bool AT__TeaStoreVariation__PidController_xcore::fromSEND () {
    if (!atSEND()) return false;
    __statesbv[ 3/(8*sizeof(int))] &= ~(1<< (3%(8*sizeof(int))));
    return true;
}

AT__TeaStoreVariation__PidController_xcore::AT__TeaStoreVariation__PidController_xcore(const string &name , AtomIPort__TeaStoreVariation__port_xt &_iport_decl__start_aipd, AtomIPort__TeaStoreVariation__port_xt &_iport_decl__then_aipd, AtomIPort__TeaStoreVariation__int_xport_xt &_iport_decl__send_aipd, AtomIPort__TeaStoreVariation__boolfloatfloat_xport_xt &_iport_decl__recv_aipd
...
BipError& AT__TeaStoreVariation__PidController_xcore::execute(PortValue &portValue) {
...
BipError& AT__TeaStoreVariation__PidController_xcore::execute(AtomExternalPort &port) {
   BipError *ret = &BipError::NoError;
...
BipError& AT__TeaStoreVariation__PidController_xcore::initialize() {
    BipError *ret = &BipError::NoError;
...
string AT__TeaStoreVariation__PidController_xcore::toString() const {
    ostringstream oss;
    bool first=true;
    if (atINIT()) {
      if (!first) oss << ", ";
    ...
    oss << "enabled_aidd=" << _id__enabled_aidd;
    if (first) first = false;
    else oss << std::endl;
    oss << "requested__time_aidd=" << _id__requested__time_aidd;
    if (first) first = false;
    else oss << std::endl;
    oss << "resulting__time_aidd=" << _id__resulting__time_aidd;
    if (first) first = false;
    else oss << std::endl;
    oss << "derivative_aidd=" << _id__derivative_aidd;
    if (first) first = false;
    else oss << std::endl;
    oss << "integral_aidd=" << _id__integral_aidd;
    if (first) first = false;
    else oss << std::endl;
    oss << "error_aidd=" << _id__error_aidd;
    if (first) first = false;
    else oss << std::endl;
    oss << "dt_aidd=" << _id__dt_aidd;
    if (first) first = false;
    else oss << std::endl;
    oss << "p_aidd=" << _id__p_aidd;
    if (first) first = false;
    else oss << std::endl;
    oss << "i_aidd=" << _id__i_aidd;
    if (first) first = false;
    else oss << std::endl;
    oss << "d_aidd=" << _id__d_aidd;
    if (first) first = false;
    else oss << std::endl;
    oss << "knob__value_aidd=" << _id__knob__value_aidd;
    if (first) first = false;
    else oss << std::endl;
    oss << "min__knob__value_aidd=" << _id__min__knob__value_aidd;
    if (first) first = false;
    else oss << std::endl;
    oss << "max__knob__value_aidd=" << _id__max__knob__value_aidd;
    if (first) first = false;
    else oss << std::endl;
    oss << "knob_aidd=" << _id__knob_aidd;

    return oss.str(); 
}

BipError& AT__TeaStoreVariation__PidController_xcore::update() {
    BipError *ret = &BipError::NoError;
...
BipError &AT__TeaStoreVariation__PidController_xcore::executeInternalTransitions() {
    BipError &ret = BipError::NoError;
...
AT__TeaStoreVariation__PidController_xcore::~AT__TeaStoreVariation__PidController_xcore() {
}

...
#include <TeaStoreVariation/AT__TeaStoreVariation__WebPageService_xcore.hpp>
...
bool AT__TeaStoreVariation__WebPageService_xcore::atINIT () const {
    return __statesbv[ 0/(8*sizeof(int))] & 1<< (0%(8*sizeof(int)));
}

bool AT__TeaStoreVariation__WebPageService_xcore::toINIT () {
    if (atINIT()) return false;
    __statesbv[ 0/(8*sizeof(int))] |= 1<< (0%(8*sizeof(int)));
    return true;
}

bool AT__TeaStoreVariation__WebPageService_xcore::fromINIT () {
    if (!atINIT()) return false;
    __statesbv[ 0/(8*sizeof(int))] &= ~(1<< (0%(8*sizeof(int))));
    return true;
}

bool AT__TeaStoreVariation__WebPageService_xcore::atRECEIVE () const {
    return __statesbv[ 1/(8*sizeof(int))] & 1<< (1%(8*sizeof(int)));
}

bool AT__TeaStoreVariation__WebPageService_xcore::toRECEIVE () {
    if (atRECEIVE()) return false;
    __statesbv[ 1/(8*sizeof(int))] |= 1<< (1%(8*sizeof(int)));
    return true;
}

bool AT__TeaStoreVariation__WebPageService_xcore::fromRECEIVE () {
    if (!atRECEIVE()) return false;
    __statesbv[ 1/(8*sizeof(int))] &= ~(1<< (1%(8*sizeof(int))));
    return true;
}

bool AT__TeaStoreVariation__WebPageService_xcore::atTHEN () const {
    return __statesbv[ 2/(8*sizeof(int))] & 1<< (2%(8*sizeof(int)));
}

bool AT__TeaStoreVariation__WebPageService_xcore::toTHEN () {
    if (atTHEN()) return false;
    __statesbv[ 2/(8*sizeof(int))] |= 1<< (2%(8*sizeof(int)));
    return true;
}

bool AT__TeaStoreVariation__WebPageService_xcore::fromTHEN () {
    if (!atTHEN()) return false;
    __statesbv[ 2/(8*sizeof(int))] &= ~(1<< (2%(8*sizeof(int))));
    return true;
}

bool AT__TeaStoreVariation__WebPageService_xcore::atSEND () const {
    return __statesbv[ 3/(8*sizeof(int))] & 1<< (3%(8*sizeof(int)));
}

bool AT__TeaStoreVariation__WebPageService_xcore::toSEND () {
    if (atSEND()) return false;
    __statesbv[ 3/(8*sizeof(int))] |= 1<< (3%(8*sizeof(int)));
    return true;
}

bool AT__TeaStoreVariation__WebPageService_xcore::fromSEND () {
    if (!atSEND()) return false;
    __statesbv[ 3/(8*sizeof(int))] &= ~(1<< (3%(8*sizeof(int))));
    return true;
}

AT__TeaStoreVariation__WebPageService_xcore::AT__TeaStoreVariation__WebPageService_xcore(const string &name , AtomIPort__TeaStoreVariation__port_xt &_iport_decl__start_aipd, AtomIPort__TeaStoreVariation__port_xt &_iport_decl__then_aipd, AtomIPort__TeaStoreVariation__boolboolint_xport_xt &_iport_decl__send_aipd, AtomIPort__TeaStoreVariation__boolint_xport_xt &_iport_decl__recv_aipd
                         
                         , AtomEPort__TeaStoreVariation__boolboolint_xport_xt &_eport_decl__send_aepd, AtomEPort__TeaStoreVariation__boolint_xport_xt &_eport_decl__recv_aepd
                         
                         , bool _comp_param__starts) :  ComponentItf(name, ATOM), Atom(name) , _iport_decl__start_aipd(_iport_decl__start_aipd), _iport_decl_pv__start_aipd(), _iport_decl__then_aipd(_iport_decl__then_aipd), _iport_decl_pv__then_aipd(), _iport_decl__send_aipd(_iport_decl__send_aipd), _iport_decl_pv__send_aipd(_id__reqAuth_aidd, _id__isNewPage_aidd, _id__idToRespondTo_aidd), _iport_decl__recv_aipd(_iport_decl__recv_aipd), _iport_decl_pv__recv_aipd(_id__requiresAuth_aidd, _id__respondToThisIDs_aidd), _eport_decl__send_aepd(_eport_decl__send_aepd), _eport_decl__recv_aepd(_eport_decl__recv_aepd), _comp_param__starts(_comp_param__starts), _transguard__1(false), _transguard__2(false), _transguard__3(false), _transguard__4(false), _transguard__5(false), __previous(-1) {
...
BipError& AT__TeaStoreVariation__WebPageService_xcore::execute(PortValue &portValue) {
...
BipError& AT__TeaStoreVariation__WebPageService_xcore::execute(AtomExternalPort &port) {
   BipError *ret = &BipError::NoError;
...
BipError& AT__TeaStoreVariation__WebPageService_xcore::initialize() {
    BipError *ret = &BipError::NoError;
...
string AT__TeaStoreVariation__WebPageService_xcore::toString() const {
    ostringstream oss;
    bool first=true;
    if (atINIT()) {
      if (!first) oss << ", ";
    ...
    oss << "respondToThisIDs_aidd=" << _id__respondToThisIDs_aidd;
    if (first) first = false;
    else oss << std::endl;
    oss << "currentRequestAnswered_aidd=" << _id__currentRequestAnswered_aidd;
    if (first) first = false;
    else oss << std::endl;
    oss << "requestsToAnswerTo_aidd=" << _id__requestsToAnswerTo_aidd;
    if (first) first = false;
    else oss << std::endl;
    oss << "requestFound_aidd=" << _id__requestFound_aidd;
    if (first) first = false;
    else oss << std::endl;
    oss << "reqAuth_aidd=" << _id__reqAuth_aidd;
    if (first) first = false;
    else oss << std::endl;
    oss << "isNewPage_aidd=" << _id__isNewPage_aidd;
    if (first) first = false;
    else oss << std::endl;
...
BipError& AT__TeaStoreVariation__WebPageService_xcore::update() {
    BipError *ret = &BipError::NoError;
...
BipError &AT__TeaStoreVariation__WebPageService_xcore::executeInternalTransitions() {
    BipError &ret = BipError::NoError;
...
AT__TeaStoreVariation__WebPageService_xcore::~AT__TeaStoreVariation__WebPageService_xcore() {
}
